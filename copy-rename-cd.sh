...
REGEX_CAPTURE_REST='^[0-9]+[[:space:]]+(.*)\.aiff$'
...
if ! [[ "$start_num" =~ ^[0-9]+$ ]]; then
    echo "Error: Starting number (-n) must be an integer."
    exit 1
fi

# --- Expand Tilde (~) in Destination Path ---
expanded_dest_dir="$dest_dir" # Default to original path
...
# Ensure the *final* destination directory exists first
if $dry_run; then
    ...
    dest_base_dir=$(dirname "$expanded_dest_dir")
     if [[ ! -d "$dest_base_dir" ]]; then
        echo "DRY RUN WARNING: Base directory '$dest_base_dir' for final destination does not exist."
     fi
else
    echo "Ensuring final destination directory exists: '$expanded_dest_dir'"
    mkdir -p "$expanded_dest_dir"
    if [[ $? -ne 0 ]]; then
        echo "Error: Could not create final destination directory '$expanded_dest_dir'."
        exit 1
    fi
fi

# Create a temporary directory *inside* the final destination
# This happens even in dry run to allow simulation of rename/move paths
temp_dir=$(mktemp -d "${expanded_dest_dir}/temp_copy_rename_XXXXXX")
if [[ $? -ne 0 ]] || [[ -z "$temp_dir" ]]; then
  echo "Error: Failed to create temporary directory in '$expanded_dest_dir'."
  exit 1
fi
echo "Created temporary directory: $temp_dir"

# Set trap to clean up temp dir on exit/interrupt - MUST be set after temp_dir is created
...
if $dry_run; then
    rsync_cmd_copy+=(-n)
    echo "DRY RUN: Simulating file copy to temp directory..."
fi
rsync_cmd_copy+=("$source_dir/" "$temp_dir/") # Source (with /) to Temp (with /)

"${rsync_cmd_copy[@]}"
rsync_exit_code=$?

if [[ $rsync_exit_code -ne 0 ]]; then
    echo "Error: rsync copy process failed with exit code $rsync_exit_code."
    if $dry_run; then echo "(Dry run mode - error check is indicative)"; fi
    exit $rsync_exit_code # Exit if copy fails
fi

if $dry_run; then
    echo "DRY RUN: File copy simulation to temp complete."
else
    echo "File copy to temp complete."
fi
echo "-------------------------------------"
...
find "$temp_dir" -maxdepth 1 -name "$RENAME_PATTERN" -print0 | sort -zV | while IFS= read -r -d $'\0' old_filepath; do
    ...
    if [[ "$old_filename" =~ $REGEX_CAPTURE_REST ]]; then
        rest_of_name="${BASH_REMATCH[1]}"

        # --- Apply 3-Digit Padding ---
        new_number_formatted=$(printf "%03d" $counter)
        # --- End Padding ---

        new_filename="${new_number_formatted}${NEW_DELIMITER}${rest_of_name}"
        # Construct the full new path *within the temporary directory*
...
# Use rsync to move files; it handles existing files/dirs better than basic mv *
# --remove-source-files makes it behave like 'mv'
rsync_cmd_move=(rsync -ah --remove-source-files) # Archive, human-readable, remove source after copy
if $dry_run; then
    rsync_cmd_move+=(-n --stats) # Dry run for move, show stats for clarity
    echo "DRY RUN: Simulating move from temp to final destination..."
fi
rsync_cmd_move+=("$temp_dir/" "$expanded_dest_dir/") # Source (temp dir contents) to Final Destination

# Execute rsync move
"${rsync_cmd_move[@]}"
rsync_move_exit_code=$?

if [[ $rsync_move_exit_code -ne 0 ]]; then
    echo "Error: rsync move process failed with exit code $rsync_move_exit_code."
    if $dry_run; then echo "(Dry run mode - error check is indicative)"; fi
    # Don't exit here, allow cleanup trap to run, but maybe warn user
    echo "Warning: Files may still be in temporary directory '$temp_dir' due to move error."
else
    if $dry_run; then
        echo "DRY RUN: File move simulation complete."
    else
...
# --- Final Cleanup ---
# The temporary directory is removed by the 'trap cleanup EXIT...' command when the script exits.
echo "Process finished."

if $dry_run; then
    echo "*** DRY RUN MODE was enabled - No actual changes were made to final destination or source. Temp dir was created and will be removed. ***"
fi
