...
# --- Script to create M4B audiobook from sorted MP3 files ---
...
# --- NOTE: Re-encodes MP3 audio to AAC                      ---
...
usage() {
  ...
  echo "  -i <input_dir>    : Directory containing sorted MP3 files (required)."
...
echo "Starting Audiobook Creation from MP3 files..."
...
# MODIFIED: Find *.mp3 files
done < <(find "$input_dir" -maxdepth 1 -name "*.mp3" -print0 | sort -zV) # Relying on sort -V

if [[ $file_count -eq 0 ]]; then echo "Error: No .mp3 files found in '$input_dir'."; exit 1; fi
echo "$file_count MP3 files processed."
...
if $dry_run; then
    ...
    echo "Starting final encoding process (re-encoding MP3->AAC, this may take a while)..."
